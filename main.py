...
def inicializar_banco():
    """Lê o arquivo schema.sql e executa a criação do banco e população inicial."""
    banco_existe = os.path.exists(NOME_BANCO)
    
    conn = conectar()
    cursor = conn.cursor()
    
    # Se o banco não existir ou estiver sem tabelas, executa o script SQL externo
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='clientes'")
    tabela_existe = cursor.fetchone()

    if not tabela_existe and os.path.exists(ARQUIVO_SCHEMA):
...
def registrar_pedido():
    ...
    try:
        ...
        while True:
            ...
            quantidade = ler_inteiro("Quantidade/Horas executadas: ", 1)
